...
import { CleanTeamDto, ITeam } from "@/domain/interfaces/ITeam";
...
export const updateTeam = async (updatedUser: ITeam) => {
  ...
  try {
    ...
    const response = await teamEndpoint.v10TeamsUpdate(
      CleanTeamDto(updatedUser),
      params
    );

    if (response?.ok) {
      return { message: successMesage.updateTeam };
    } else {
